...
pub async fn get_courses_for_tutor(
    ...
    let filtered_courses = app_state
        ...
        .map(convert_local_time) // 현재 시간으로 변경
        .collect::<Vec<Course>>();

    if filtered_courses.len() > 0 {
        HttpResponse::Ok().json(filtered_courses)
    } else {
        HttpResponse::Ok().json("No courses".to_string())
    }
}

// local time으로 컨버팅 - 커스텀함~
fn convert_local_time(course: Course) -> Course {
    Course {
        posted_time: course.posted_time.map(|t| Local.from_utc_datetime(&t).naive_local()),
        ..course
    }
}
...
pub async fn get_course_detail(
    ...
    let (tutor_id, course_id) : (i32, i32) = params.into_inner();
